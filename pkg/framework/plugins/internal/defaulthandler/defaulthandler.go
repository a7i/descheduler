...
// DefaultHandler is the base plugin handler, which defines the default extension points of the
// pre-baked evictor or deleter that is shipped.
...
func New(args runtime.Object, handle frameworktypes.Handle) (frameworktypes.Plugin, error) {
	defaultHandlerArgs, ok := args.(*DefaultHandlerArgs)
	if !ok {
		return nil, fmt.Errorf("want args to be of type defaultHandlerArgs, got %T", args)
	}

	ev := &DefaultHandler{}
	ev.handle = handle
	ev.args = defaultHandlerArgs

	if defaultHandlerArgs.EvictFailedBarePods {
	...
	if !defaultHandlerArgs.EvictSystemCriticalPods {
		ev.constraints = append(ev.constraints, func(pod *v1.Pod) error {
		...
		if defaultHandlerArgs.PriorityThreshold != nil && (defaultHandlerArgs.PriorityThreshold.Value != nil || len(defaultHandlerArgs.PriorityThreshold.Name) > 0) {
			thresholdPriority, err := utils.GetPriorityValueFromPriorityThreshold(context.TODO(), handle.ClientSet(), defaultHandlerArgs.PriorityThreshold)
			if err != nil {
	...
	if !defaultHandlerArgs.EvictLocalStoragePods {
		ev.constraints = append(ev.constraints, func(pod *v1.Pod) error {
	...
	if defaultHandlerArgs.IgnorePvcPods {
		ev.constraints = append(ev.constraints, func(pod *v1.Pod) error {
	...
	selector, err := metav1.LabelSelectorAsSelector(defaultHandlerArgs.LabelSelector)
	if err != nil {
		return nil, fmt.Errorf("could not get selector from label selector")
	}
	if defaultHandlerArgs.LabelSelector != nil && !selector.Empty() {
		ev.constraints = append(ev.constraints, func(pod *v1.Pod) error {
